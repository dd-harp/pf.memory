...
We let $h(t)$ denote the FoI. To construct $h(t)$, we let $\bar h$ be a parameter that sets the overall level of exposure. We assume exposure has a seasonal pattern, $S(t)$, and a temporal trend, $T(t)$, and $$h(t) = \bar h \cdot S(t) \cdot T(t).$$ 
...
We can thus compare exposure and infection for cohorts of different ages, of the same age at different times. 

To pressure test the models, we developed a class of functions to simulate exposure with respect to time or age. Supplement 1 includes a highly configurable and extensible library of FoI functions, output from this R-markdown file as `Supplement1.R` using `purl` from the R package `knitr`. 
...
plotFoI= function(h, FoIpar, tau=0, Tmax=5*365, clrs=NULL){
  ...
  tms = 1:Tmax
  tm = tms/365 
  plot(tm, 365*FoI(tms, FoIpar, tau, hhat=h), type = "n", 
       xlab = "Host Age (Years)", ylab = "aFoI") 
  for(i in 1:L) lines(tm, 365*FoI(tms,FoIpar, tau, hhat=h[i]), col = clrs[i])
}
```
